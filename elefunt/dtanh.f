...
C
C         DABS, DLOG, DMAX1, FLOAT, DSQRT, DTANH
C
C
...
C
     ...
     1        MAXEXP,MINEXP,N,NEGEP,NGRD
      REAL*8 A,AIT,ALBETA,B,BETA,BETAP,C,D,DEL,EPS,EPSNEG,EXPON,HALF,
     1     ONE,REN,R6,R7,W,X,XL,XMAX,XMIN,XN,X1,Y,Z,ZERO,ZZ
C
      IOUT = 6
     ...
     1            MAXEXP,EPS,EPSNEG,XMIN,XMAX)
      ...
      D = DLOG(2.0D0) + (AIT+ONE) * DLOG(BETA) * HALF
      N = 2000
      XN = DFLOAT(N)
      I1 = 0
C-----------------------------------------------------------------
...
C
      ...
      WRITE (IOUT,1100)
      STOP
 1000 FORMAT(21H1TEST OF DTANH(X) VS ,
     1  54H(DTANH(X-1/8)+DTANH(1/8))/(1+DTANH(X-1/8)DTANH(1/8))   //)
 1010 FORMAT(I7,47H RANDOM ARGUMENTS WERE TESTED FROM THE INTERVAL /
     1 6X,1H(,E15.4,1H,,E15.4,1H)//)
 1011 FORMAT(20H DTANH(X) WAS LARGER,I6,7H TIMES, /
 ...
 1030 FORMAT(55H THE IDENTITY   DTANH(-X) = -DTANH(X)   WILL BE TESTED.
     1      //8X,1HX,9X,12HF(X) + F(-X)/)
 1031 FORMAT(53H THE IDENTITY DTANH(X) = X , X SMALL, WILL BE TESTED.//
     1       8X,1HX,9X,8HX - F(X)/)
 1032 FORMAT(53H THE IDENTITY DTANH(X) = 1 , X LARGE, WILL BE TESTED.//
     1       8X,1HX,9X,8H1 - F(X)/)
 1035 FORMAT(43H TEST OF UNDERFLOW FOR VERY SMALL ARGUMENT. /)
 1040 FORMAT(52H THE FUNCTION DTANH WILL BE CALLED WITH THE ARGUMENT,
     1   E15.7)
 1060 FORMAT(2E15.7/)
 1061 FORMAT(6X,7H DTANH(,E13.6,3H) =,E13.6/)
 1100 FORMAT(25H THIS CONCLUDES THE TESTS )
C     ---------- LAST CARD OF DTANH TEST PROGRAM ----------
      END
